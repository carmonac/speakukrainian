 ...
 * a **Cloud Storage path segment**, where `__name__` is a perfectly legal
 * object name and a `|` is not. Replacing it with the shared schema would swap
 * one rule for another that happens to have the same alphabet and a different
 * reason, so the two are left to move independently (ADR-018).
 */
